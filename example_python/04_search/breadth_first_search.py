...
-----------------------------------------------------------------------
幅優先探索（Breadth-First Search; BFS）のサンプルプログラム。

このプログラムでは木構造のデータを使用して幅優先探索を
実行しています。
...
# 幅優先探索（全てのノードを探索）
def bfs_entire_tree(root):
    queue = [root]

    while queue:
        node = queue.pop(0)  # 先頭を取り出す
...
# 幅優先探索で全てのノードを探索
print("-" * 30)
print("幅優先探索で全てのノードを順番に探索します。")
bfs_entire_tree(tree.root)

# ---------------------------------------------------------------------
# 幅優先探索で指定したノードを探索
print("-" * 30)
target = "E"
print(f"幅優先探索でノード {target} を探索します。")
node_found = bfs_target(tree.root, target)
